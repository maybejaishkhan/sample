...
The Publish stage expects each scanner to write its output under a well-known
file name (configured in pipelines/variables/security.yml):

    trufflehog.json  -> parser.trufflehog
    semgrep.json     -> parser.semgrep
    msdo.sarif       -> parser.msdo
    zap.json         -> parser.zap
...
from parser.msdo import parse as parse_msdo
